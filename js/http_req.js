...
function main(sources){
	const clickEvent$ = sources.DOM.select('.get-first')
						.events('click');
	...
	const response$$ = sources.HTTP
		.filter(response$ => response$.request.url == 'http://jsonplaceholder.typicode.com/users/2');
		...
		DOM: firstUser$.map(firstUser =>
			div([
				...
				firstUser == null? null: div('.user-details', [
					...
					a('.user-website', {href: 'http://'+firstUser.website}, firstUser.website)
				])
			])
		),
		HTTP: request$
